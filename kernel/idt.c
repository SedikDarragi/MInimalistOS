...
void pic_remap(void) {
    ...
    // Mask all interrupts initially except the ones we'll unmask in kmain
    // 0xFB allows the cascade from PIC2; 0xFF masks all on PIC2
    outb(PIC1_DATA, 0xFB); 
    outb(PIC2_DATA, 0xFF);
}
...
void idt_init() {
    ...
    // Set up all ISRs (CPU exceptions 0-31)
    ...
    // Set up all 16 IRQs (hardware interrupts 32-47)
